...
from PySide6.QtCore import Qt, QThread, Signal, QTimer, QPropertyAnimation, QEasingCurve, QUrl, QObject, Slot, QPointF, QRectF, QDateTime
                        ...
                        for i, firm in enumerate(firms):
                            ...
                            if self.db:
                                try:
                                    ...
                                    print(f"✅ DB'ye kaydedildi: {firm.get('name', 'İsimsiz')}")  # DEBUG
                                except Exception as e:
...
class WorkerThread(QThread):
    ...
    def send_campaign(self):
        ...
        try:
            ...
            self.error.emit("Email ayarları bulunamadı!")
            return
        ...
        for i, firm in enumerate(firms):
            ...
            for email_data in sorted_emails[:3]:
                ...
                self.db.save_email_log(firm['id'], email_data['email'], result)
...
class MainWindow(QMainWindow):
    ...
    def create_search_tab(self):
        ...
        self.analyze_selected_btn.clicked.connect(lambda: print("🔧 DEBUG: Analiz butonu tıklandı!") or self.analyze_selected_firms())
        ...
        self.add_to_campaign_btn.clicked.connect(lambda: print("🔧 DEBUG: Kampanya butonu tıklandı!") or self.add_selected_to_campaign())
    ...
    def add_new_firm(self):
        ...
        QMessageBox.information(self, "Bilgi", 
            "Yeni firma ekleme özelliği yakında eklenecek!\n\n"
            "Şu anda firmalar Google Maps araması ile ekleniyor.")
    ...
    def create_campaign_tab(self):
        ...
        # Hazır şablonlar
        template_layout.addWidget(QLabel("Hazır Şablon:"))
    ...
    def create_tracking_tab(self):
        ...
        # Kart widget'ları
        self.sent_card = ModernCard("Gönderilen", "0", "rgba(52, 152, 219", "📤")
        self.opened_card = ModernCard("Açılan", "0", "rgba(46, 204, 113", "👁️")
        self.clicked_card = ModernCard("Tıklanan", "0", "rgba(241, 196, 15", "🖱️")
        self.replied_card = ModernCard("Yanıtlanan", "0", "rgba(155, 89, 182", "💬")
        self.bounce_card = ModernCard("Geri Dönen", "0", "rgba(231, 76, 60", "⚠️")
        self.unsubscribe_card = ModernCard("Abonelikten Çıkan", "0", "rgba(149, 165, 166", "🚫")
    ...
    def update_tracking_dashboard(self):
        """Tracking dashboard'unu güncelle"""
        period = self.period_filter.currentText()
        campaign = self.campaign_filter.currentText()
        
        # İstatistikleri güncelle
        stats = self.db.get_tracking_statistics(period, campaign)
        
        self.sent_card.update_value(stats.get('sent', 0))
        self.opened_card.update_value(stats.get('opened', 0))
        self.clicked_card.update_value(stats.get('clicked', 0))
        self.replied_card.update_value(stats.get('replied', 0))
        self.bounce_card.update_value(stats.get('bounced', 0))
        self.unsubscribe_card.update_value(stats.get('unsubscribed', 0))
        
        # HTML Dashboard'u güncelle
        self.load_tracking_dashboard_html(stats)
        
        # Isı haritasını güncelle
        self.load_heatmap_html(stats)
        
        # Cihaz analizini güncelle
    ...
    def export_tracking_report(self):
        """Tracking raporunu dışa aktar"""
        file_name, _ = QFileDialog.getSaveFileName(
            self, "Tracking Raporu Kaydet",
            f"tracking_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
    ...
    def create_firms_tab(self):
        ...
        self.all_firms_table.setHorizontalHeaderLabels([
            "✓", "Firma Adı", "Rating", "Email Sayısı", "Website", "Telefon", "Sektör", "Analiz", "Kampanya", "Detay", "İşlemler"
        ])
    ...
    def create_settings_tab(self):
        ...
        self.tracking_url_input.setPlaceholderText("http://raspberry-pi-ip:5000")
        tracking_layout.addWidget(self.tracking_url_input, 0, 1)
        
        tracking_info = QLabel("ℹ️ Email açılma takibi için Raspberry Pi'de tracking server çalıştırın")
    ...
    def create_whatsapp_tab(self):
        ...
        # Modern tarayıcı özelliklerini aktifleştir
    ...
    def update_selected_count(self):
        ...
        count = 0
        total_rows = self.firms_table.rowCount()
        ...
        self.send_to_whatsapp_btn.setEnabled(is_enabled)
        self.send_to_call_btn.setEnabled(is_enabled)
        print(f"🔧 DEBUG: Butonlar {'aktif' if is_enabled else 'pasif'}")  # DEBUG
    ...
    def analyze_selected_firms(self):
        ...
        for row in range(self.firms_table.rowCount()):
            checkbox = self.firms_table.cellWidget(row, 0)
            if checkbox and checkbox.isChecked():
                selected_firms.append(self.current_firms[row])
    ...
    def on_analyze_finished(self, data):
        ...
        # Tabloyu güncelle
        self.update_firm_in_table(firm_data)
    ...
    def update_firm_in_table(self, firm_data):
        """Firma verisini tabloda güncelle"""
    ...
    def add_selected_to_campaign(self):
        ...
        firms_without_email = [f for f in selected_firms if not f.get('emails')]
    ...
    def update_campaign_firms_list(self):
        ...
        for firm in self.selected_firms:
            ...
            if email_count == 0:
                ...
                item.setBackground(QColor(100, 100, 30))
            else:
    ...
    def start_campaign(self):
        ...
        if self.test_mode_check.isChecked():
            firms_to_send = self.selected_firms[:3]
    ...
    def load_all_firms(self):
        ...
        try:
            ...
            for i, firm in enumerate(firms):
                ...
                # Email Sayısı (Kolon 3)
                email_count = firm.get('email_count', 0)
                self.all_firms_table.setItem(i, 3, QTableWidgetItem(str(email_count)))
    ...
    def send_follow_up_email(self, email_data):
        ...
        if result['success']:
            ...
            self.db.save_email_log(email_data['firm_id'], email_data.get('to_email'), result)
        else:
    ...
    def load_settings(self):
        ...
        try:
            ...
            # UI elementleri varsa ayarları yükle
            if hasattr(self, 'google_api_input'):
                self.google_api_input.setText(settings.get("google_api_key", ""))
            if hasattr(self, 'openai_api_input'):
                self.openai_api_input.setText(settings.get("openai_api_key", ""))
            if hasattr(self, 'snov_id_input'):
                self.snov_id_input.setText(settings.get("snov_client_id", ""))
            if hasattr(self, 'snov_secret_input'):
                self.snov_secret_input.setText(settings.get("snov_client_secret", ""))
            if hasattr(self, 'email_input'):
                self.email_input.setText(settings.get("smtp_email", ""))
            if hasattr(self, 'email_password_input'):
                self.email_password_input.setText(settings.get("smtp_password", ""))
            ...
            if hasattr(self, 'tracking_url_input'):
                self.tracking_url_input.setText(settings.get("tracking_url", ""))
            # WhatsApp ayarları yükleme kaldırıldı
            
            if hasattr(self, 'calendar_client_id_input'):
                self.calendar_client_id_input.setText(settings.get("calendar_client_id", ""))
                self.calendar_client_secret_input.setText(settings.get("calendar_client_secret", ""))
                self.calendar_redirect_uri_input.setText(settings.get("calendar_redirect_uri", "http://localhost:8080/callback"))
